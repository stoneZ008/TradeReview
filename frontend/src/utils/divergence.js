export function detectMACDDivergence(data, lookback = 20) {
  ...
  for (let i = lookback; i < data.length; i++) {
    ...
    for (let j = 1; j <= 5; j++) {
      if (i - j >= 0 && data[i - j].close >= data[i].close) {
        isPriceHigh = false;
        break;
      }
      if (i + j < data.length && data[i + j].close >= data[i].close) {
        isPriceHigh = false;
        break;
      }
    }
    ...
    if (isPriceHigh) {
      for (let j = i - lookback; j < i - 5; j++) {
        let isPrevHigh = true;
        for (let k = 1; k <= 5; k++) {
          if (j - k >= 0 && data[j - k].close >= data[j].close) {
            isPrevHigh = false;
            break;
          }
          if (j + k < data.length && data[j + k].close >= data[j].close) {
            isPrevHigh = false;
            break;
          }
        }
