...
def authenticate_user(username, password):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, password_hash, is_active FROM users WHERE username = ? OR email = ?", (username, username)
    )
    row = cursor.fetchone()
    ...
    return {"access_token": access_token, "refresh_token": refresh_token, "user": get_user_by_id(user_id)}, None
...
def get_user_by_id(user_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, username, email, is_active, trial_end_at, last_login, created_at FROM users WHERE id = ?",
        (user_id,),
    )
    row = cursor.fetchone()
    conn.close()
