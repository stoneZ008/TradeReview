...
def seed_default_data():
    ...
    # 插入默认行业数据
    industries = [("光模块", "📡"), ("芯片", "💾"), ("服务器/算力", "🖥️"), ("新能源", "⚡")]
    ...
    for industry in industries:
        ...
        for sub in sub_industries:
            if sub["industry_id"] == industry["id"]:
                ...
                children.append({"id": str(sub["id"]), "name": sub["name"], "companies": sub_companies})

        result.append(
            {"id": str(industry["id"]), "name": industry["name"], "icon": industry["icon"], "children": children}
        )
...
def update_industry(industry_id, name, icon):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE industries SET name=?, icon=?, updated_at=CURRENT_TIMESTAMP WHERE id=?", (name, icon, industry_id)
    )
    conn.commit()
    conn.close()
...
def update_sub_industry(sub_industry_id, name):
    ...
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("UPDATE sub_industries SET name=?, updated_at=CURRENT_TIMESTAMP WHERE id=?", (name, sub_industry_id))
    conn.commit()
    conn.close()
