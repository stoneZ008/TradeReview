...
class BOLLSupport(TradingStrategy):
    ...
    def generate_signals(self, df):
        ...
        # 触及下轨
        touch_lower = df["low"] <= df["boll_lower"]  # noqa: F841
...
class MA20Support(TradingStrategy):
    ...
    def generate_signals(self, df):
        ...
        # 接近MA20（偏差<1%）
        near_ma20 = abs(df["close"] - df["ma20"]) / df["ma20"] < 0.01  # noqa: F841
...
class BottomReversal(TradingStrategy):
    ...
    def generate_signals(self, df):
        ...
        for i in range(self.lookback, len(df)):
            ...
            macd_hist_rising = df["macd_hist"].iloc[i] > df["macd_hist"].iloc[i - 1]  # noqa: F841
            ...
            rsi_recovering = (df["rsi"].iloc[i] < 40) and (df["rsi"].iloc[i] > df["rsi"].iloc[i - 1])
...
class BOLLResistance(TradingStrategy):
    ...
    def generate_signals(self, df):
        ...
        # 触及上轨
        touch_upper = df["high"] >= df["boll_upper"]  # noqa: F841
...
def generate_trading_signals(df, config=None):
    ...
    composite = CompositeStrategy(
        buy_threshold=config.get("buy_threshold", 0.08), sell_threshold=config.get("sell_threshold", 0.08)
    )
