...
SCHEMA_COLUMNS = {
    ...
    "watchlist_signal_snapshots": [
        ...
        ("last_signal_v2", "INTEGER DEFAULT 0"),
        ("last_signal_date_v2", "TEXT"),
        ("has_signal_today_v2", "INTEGER DEFAULT 0"),
        ("error", "TEXT"),
        ("created_at", "TIMESTAMP DEFAULT CURRENT_TIMESTAMP"),
    ],
    "audit_logs": [
...
def seed_initial_data():
    ...
    for perm_id in super_admin_perms:
        cursor.execute(
            "INSERT INTO role_permissions (role_id, permission_id) VALUES (?, ?)", (role_map["super_admin"], perm_id)
        )
    ...
    user_free_perms = ["stock:read", "watchlist:read", "watchlist:write", "profile:read", "profile:write"]
    ...
    plans = [
        ...
        (
            ...
            0,
            -1,
            json.dumps({"indicators": True, "dao_page": True, "priority_support": True}),
            "企业版（知识星球开通）",
        ),
    ]
    for name, monthly_price, yearly_price, max_backtests, features, desc in plans:
        cursor.execute(
            ...
            INSERT INTO subscription_plans
                (name, monthly_price, yearly_price, max_backtests_monthly, features_json, description)
            VALUES (?, ?, ?, ?, ?, ?)
        """,
