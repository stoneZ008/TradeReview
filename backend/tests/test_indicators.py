...
def test_calculate_sma():
    """测试均线计算"""
    data = pd.DataFrame({"close": [100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110]})
...
def test_calculate_boll():
    """测试布林带计算"""
    data = pd.DataFrame({"close": np.random.rand(50) * 100 + 100})
...
def test_calculate_rsi():
    """测试RSI计算"""
    data = pd.DataFrame({"close": np.random.rand(30) * 100 + 100})
...
def test_calculate_rsi_all_gains():
    ...
    data = pd.DataFrame({"close": [100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115]})
